    ...
    def error(self, *args):
        ...
        match errorCode :
            ...
            case  165 | 2104 | 2106 | 2110 | 2119 | 2152 | 2158:
                logger.info(f"IBKR Info event: {args} -> (reqId, timestamp, errorCode, errorString, advancedOrderRejectionJson)")
            # WARNING
            case range(1100, 1300) | 2103 | 2105 | 2137:
                logger.warning(f"IBKR Warning event: {args} -> (reqId, timestamp, errorCode, errorString, advancedOrderRejectionJson)")
            # ERROR
            case range(100, 1000) | range(2100, 11000):
                logger.error(f"IBKR Error event: {args} -> (reqId, timestamp, errorCode, errorString, advancedOrderRejectionJson)")
            case _:
                logger.critical(f"IBKR Critical event: {args} -> (reqId, timestamp, errorCode, errorString, advancedOrderRejectionJson)")
