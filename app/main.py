...
from app.services.ContractData import ContractDataService
...
def SetupLogger():
    ...
    # ! [ewrapperimpl]
    def __init__(self):
        EWrapper.EWrapper.__init__(self)
...
# class TradeApp(wrapper.EWrapper, EClient):
#     def __init__(self):
...
class TradeApp(EClient, EWrapper):
    ...
    def tickPrice(self, reqId: int, tickType, price: float, attrib: TickAttrib):
        print(f"Tick Price. Ticker Id: {reqId}, tickType: {TickTypeEnum.toStr(tickType)}, Price: {price}, attrib: {attrib}")

        # Initialize other services here
        # self.contract_data_service = ContractDataService()
...
def main():
    ...
    logging.info("config : " + config.__str__())
    ...
    app = TradeApp()
    app.connect(config["IBKR"]["HOST"], config["IBKR"]["PORT"], config["IBKR"]["CLIENTID"])
